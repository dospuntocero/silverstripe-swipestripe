	<table class="table table-bordered">
	  <thead>
	    <tr>
	      ...
	      <th><% _t('Order.UNITPRICE','Unit Price') %> ($Total.Currency)</th>
	      <th><% _t('Order.QUANTITY','Quantity') %></th>
	      <th><% _t('Order.SUBTOTAL','Sub Total') %> ($Total.Currency)</th>
	    </tr>
	  </thead>
	  <tbody>
	    <% control Items %>
      ...
      <% end_if %>
	    ...
	    <tr>
        <td colspan="4" class="row-header"><% _t('Order.SUBTOTAL','Sub Total') %></td>
        <td>$SubTotal.Nice</td>
      </tr>
